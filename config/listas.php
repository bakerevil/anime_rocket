...
class listas extends mysqli
{
    ...
    public function get_listas($texto = ""){
        $consulta = "SELECT s.id, s.titulo, s.thumbnail, s.sipnosis, s.capitulos, s.fecha_insercion, s.votos, s.anio, s.l_tipo, s.l_categoria, c.status FROM listas s LEFT JOIN status c ON s.l_status = c.id ";
        if($texto != ""){
